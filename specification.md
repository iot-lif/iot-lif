...
In one previous effort ([@previouswork]), our project lead and several
other collaborators performed a study of the efficacy and complexity of
...
![These are the ResNet and VGG model sizes and accuracies from the
project lead's previous paper ([@previouswork]). They were the
best-performing and also largest models in the project. We will be
aiming to get as close as possible to these accuracies with less
computationally-expensive models and reduced
precision.](resnet_vgg_accuracies.png){width="\textwidth"}

SNN with Deep Continuous Local Learning {#snn-with-deep-continuous-local-learning .unnumbered}
---------------------------------------
...
[@DBLP:journals/corr/abs-1811-10766]. It provides a novel way to train
SNNs, which are event or spike-based networks that are naturally
equipped to process temporal data (which can be represented as spikes
over time) and that are theoretically more powerful than artificial
neural networks because of the differences in their neuron design. The
problem with SNNs is that spikes over time are not differentiable, so
they are difficult to train with the standard optimization methods of
today (e.g. stochastic gradient descent) which require the ability to
compute gradients of the loss function with respect to every parameter
in the network.
...
![An illustration of the DECOLLE feedforward SNN architecture over two
timesteps. Each blue block represents a state variable, each column
represents a timestep, and the orange diamonds are auxiliary losses. The
arrows represent the flow of information (orange arrows represent the
flow of gradients). As you can see, information from one time step is
fed into the next time step, which helps the network model the temporal
aspect of the data.](snn_feedforward.png){width="55%"}

Project Parts {#project-parts .unnumbered}
-------------
...
-   Implementing a larger spiking neural network for RadioML based on
    ...
    architecture used in [@previouswork].
...
Further Extensions {#further-extensions .unnumbered}
------------------
...
-   [**An SNN network for RadioML trained using
    DECOLLE:**]{.sans-serif}\
    Based on the dcll Python library provided with
    [@DBLP:journals/corr/abs-1811-10766], we plan to create a model
    trained using the RadioML dataset for modulation classification.
    This model will be evaluated on accuracy.

-   [**A quantized version of the SNN network for
    RadioML:**]{.sans-serif}\
...
section [\[sec:schedule\]](#sec:schedule){reference-type="ref"
reference="sec:schedule"}.
...
Training Time / Hyperparameter Tuning {#training-time-hyperparameter-tuning .unnumbered}
-------------------------------------
...
Software Components {#software-components .unnumbered}
-------------------

Development for this project is mostly software-based. These are the
main software components that we will use:

-   **[dcll]{.sans-serif}**, a Python library built on top of PyTorch
    implementing spiking neural networks and the DECOLLE learning rule

-   **[Brevitas]{.sans-serif}**, a Python library based on PyTorch for
    quantization-aware training of neural networks

-   **[Vivado HLS]{.sans-serif}**, a software package for high level
    synthesis and simulation of hardware designs for FPGAs
...
[\[milestone\_summary\]](#milestone_summary){reference-type="ref"
reference="milestone_summary"} provides a summary, while a more detailed
description can be found below.

[\[sec:schedule\]]{#sec:schedule label="sec:schedule"}

  **[Week]{.sans-serif}**   **[Type]{.sans-serif}**   **[Description]{.sans-serif}**                          **[]{.sans-serif}**
  ------------------------- ------------------------- ------------------------------------------------------- ---------------------
  ...
  : Summary of milestone/deliverable schedule

[\[milestone\_summary\]]{#milestone_summary label="milestone_summary"}
...
**[Simon]{.sans-serif}**:
  ...
  --- ------------------------------------------------------------------------------
  :   Train SNN (DCLL lib) with MNIST dataset.
  :   Have an SNN network that can be run on the MNIST dataset on the GPU cluster.
  --- ------------------------------------------------------------------------------

[**Owen**]{.sans-serif}:

  --- --------------------------------------------------------------------------------
  :   Preprocess and load RadioML dataset for SNN.
  :   Have the RadioML data on the GPU cluster in a form that can be fed to the SNN.
  --- --------------------------------------------------------------------------------
...
[**Simon**]{.sans-serif}:
  ...
  --- --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  :   Get familiar with Brevitas library.
  :   Written report including a description of which functionality Brevitas offers, how the codebase of Brevitas is organized, how it is used for regular neural networks and outline on where extensions for spiking neural networks will have to be made.
  --- --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

[**Owen**]{.sans-serif}:

  --- -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  :   Get familiar with the DCLL codebase, determine which parts of the code will need to be changed in order to create a VGG-like SNN architecture using DCLL modules.
  :   Written report which includes a high-level overview of the different parts of the code and a description of which specific files and functions will need to be changed or utilized in order to define a VGG-like architecture for use with RadioML.
  --- -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
[**Simon**]{.sans-serif}:
  ...
  --- ------------------------------------------------------------------------------------------------------------------------------------
  :   Extend Brevitas for usage with spiking neural networks from dcll library.
  :   Have a trained quantized version of the MNIST network for different bit sizes (16-, 8- and 4-bit) and report on accuracy achieved.
  --- ------------------------------------------------------------------------------------------------------------------------------------

[**Owen**]{.sans-serif}:

  --- ---------------------------------------------------------------------------------------------------------------------
  :   Adapt SNN for RadioML using DCLL PyTorch library.
  :   Have a first SNN modeled after VGG that can be trained on RadioML data (ignoring actual performance at this point).
  --- ---------------------------------------------------------------------------------------------------------------------

Week 7 - Deliverable {#week-7---deliverable .unnumbered}
====================

[**Owen & Simon**]{.sans-serif}:

  --- ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  :   Hyperparameter tuning of SNN for RadioML
  ...
  :   Classify RadioML data by modulation with at least, say, 60% accuracy on the test set (which corresponds to the worst performance reported in the previous work).
  :   Report on architecture, implementation and hyperparameters for the model and discuss resulting accuracy.
  --- ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
[**Simon**]{.sans-serif}:
  ...
  --- -----------------------------------------------------------------------------------------------------------
  :   Adapt RadioML SNN for quantization in Brevitas.
  :   Have a trained quantized version of RadioML SNN in Brevitas (without focusing on accuracy at this point).
  --- -----------------------------------------------------------------------------------------------------------

[**Owen**]{.sans-serif}:

  --- --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  :   Tuning of architecture parameters for RadioML SNN
  ...
  :   Have optimized RadioML SNN with highest achieved accuracy, update report with results.
  --- --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

Week 9 - Deliverable {#week-9---deliverable .unnumbered}
====================

[**Owen & Simon**]{.sans-serif}:

  --- ------------------------------------------------------------------------------------------------------
  :   Optimize quantized version of RadioML SNN.
      Experiment with the Brevitas model for RadioML to reduce model size while maintaining high accuracy.
  :   Have optimized quantized RadioML SNN with highest achieved accuracy.
  :   Report on architecture, implementation and hyperparameters for quantized RadioML SNN.
  --- ------------------------------------------------------------------------------------------------------

Week 10 - Deliverable {#week-10---deliverable .unnumbered}
=====================
...
**[Owen & Simon]{.sans-serif}**:
  ...
  :   Submit final report and video addressing project progress, challenges, solutions and results.
  --- -----------------------------------------------------------------------------------------------
